...
internal fun KalendarFirey(
    ...
    val startDayOfMonth = LocalDate.parse("$currentYear-$monthValue-01")
    ...
    Column(
        ...
        LazyVerticalGrid(
            ...
            content = {
                if (showLabel) {
                    items(java.time.DayOfWeek.entries.toTypedArray()) { item ->
                        Text(
                            modifier = Modifier,
...
private fun calculateDay(day: Int, currentMonth: Month, currentYear: Int): LocalDate {
    ...
    return LocalDate.parse("$currentYear-$monthValue-$dayValue")
}
